...
class ScreenFacts(BaseModel):
    ...
    """
    context: str = Field(default="", description="What app/screen this is, e.g. 'Gmail showing an opened email'.")
    brand_claimed: str = Field(default="", description="Who the content claims to be from, e.g. 'Luma / Weights & Biases'.")
    sender_name: str = ""
    sender_address: str = ""
    subject: str = ""
    body_summary: str = ""
    links: list[str] = Field(default_factory=list)
    call_to_actions: list[str] = Field(default_factory=list, description="Buttons/links urging action, e.g. 'View Event'.")
    requests: list[str] = Field(default_factory=list, description="What it asks the user to do (download app, pay, log in...).")
    urgency_signals: list[str] = Field(default_factory=list, description="Pressure tactics, deadlines, threats.")
    notable_observations: list[str] = Field(default_factory=list)
    raw_text: str = Field(default="", description="Key text for vector search / comparison.")
...
class ResearchFindings(BaseModel):
    ...
    domain_assessment: str = Field(default="", description="What research says about the sender domain/brand.")
    legitimacy_indicators: list[str] = Field(default_factory=list)
    scam_indicators: list[str] = Field(default_factory=list)
    web_findings: list[str] = Field(default_factory=list, description="Relevant facts found via web search.")
    research_notes: str = ""
...
class ScamVerdict(BaseModel):
    ...
    advice: str = Field(default="", description="Plain-language explanation for a non-technical user.")
...
class ScanResult(BaseModel):
    ...
    trusted_sender: Optional[bool] = None  # True/False if the sender is on the trust list
    # Transparency: surface the intermediate stages (great for the demo + Weave).
    facts: Optional[ScreenFacts] = None
...
class ContactRequest(BaseModel):
    identifier: str = Field(description="Email address or domain, e.g. 'anna@luma-mail.com' or 'luma-mail.com'.")
    trusted: bool
